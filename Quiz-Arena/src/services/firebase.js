...
const firebaseConfig = {
  apiKey:
    ...
    "AIzaSyCBLYywuVkrkPGMIWI0UReVxDbKQrnkjV4",
  authDomain:
    import.meta.env.VITE_FIREBASE_AUTH_DOMAIN ||
    "quiz-battle-7538e.firebaseapp.com",
  projectId:
    import.meta.env.VITE_FIREBASE_PROJECT_ID ||
    "quiz-battle-7538e",
  storageBucket:
    import.meta.env.VITE_FIREBASE_STORAGE_BUCKET ||
    "quiz-battle-7538e.firebasestorage.app",
  messagingSenderId:
    import.meta.env.VITE_FIREBASE_MESSAGING_SENDER_ID ||
    "980118535845",
  appId:
    import.meta.env.VITE_FIREBASE_APP_ID ||
    "1:980118535845:web:99338394d100a8f136d74c"
};
...
try {
  ...
  console.warn(
    "Firebase initialization warning (Using multi-tab fallback mode):",
    error.message
  );
}
...
const channel =
  typeof BroadcastChannel !== 'undefined'
    ? new BroadcastChannel('quiz_battle_events')
    : null;
...
export const saveGameSessionToFirebase = async (
  gamePin,
  sessionData
) => {
  if (isFirebaseConfigured && db) {
  ...
  localStorage.setItem(
    `${FALLBACK_KEY_PREFIX}${gamePin}`,
    JSON.stringify(sessionData)
  );

  broadcastLocalEvent('GAME_SESSION_UPDATED', {
    gamePin,
    sessionData
  });
};
...
export const getGameSessionFromFirebase = async (gamePin) => {
  // Try Firebase first
  if (isFirebaseConfigured && db) {
    try {
      const gameRef = doc(db, 'games', gamePin);
      ...
      console.error("Firestore getGameSession error:", e);
    }
  }
...
export const updateGameSessionInFirebase = async (
  gamePin,
  updateFields
) => {
  if (isFirebaseConfigured && db) {
    try {
      ...
      console.error(
        "Firestore updateGameSession error:",
        e
      );
      throw e;
    }
...
export const savePlayerToFirebase = async (
  ...
  broadcastLocalEvent('PLAYER_JOINED', {
    gamePin,
    player,
    players
  });
...
export const savePlayerResponseToFirebase = async (
  gamePin,
  responseData
) => {
  ...
  const payload = {
    ...
    timeTakenMs: Number(
      responseData.timeTakenMs || 0
    ),
    pointsEarned: Number(
      responseData.pointsEarned || 0
    ),
    gamePin,
    timestamp: Date.now()
  };
  ...
  if (isFirebaseConfigured && db) {
    try {
      ...
      console.error(
        "Firestore savePlayerResponse error:",
        e
      );
      throw e;
    }
  ...
  // Multi-tab broadcast
  broadcastLocalEvent('PLAYER_ANSWERED', {
    gamePin,
...
export const subscribeToGameSession = (
  ...
  const handleBroadcast = (event) => {
    if (
      event.data?.payload?.gamePin === gamePin &&
      event.data?.payload?.sessionData
    ) {
      callback(
        event.data.payload.sessionData
      );
  ...
  if (isFirebaseConfigured && db) {
    try {
      ...
      unsubscribeFirestore = onSnapshot(
        ...
        (err) =>
          console.warn(
            "Firestore players listener error:",
            err
          )
      );
    } catch (e) {
      console.warn(
        "Firestore players error:",
        e
      );
    }
  }
